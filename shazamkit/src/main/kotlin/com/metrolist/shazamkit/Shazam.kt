...
import com.metrolist.shazamkit.models.RapidApiShazamResponse
...
object Shazam {
    ...
    /**
     * Recognize music from raw audio bytes using RapidAPI Shazam
     * @param audioData Raw audio data (can be base64 encoded)
     */
    suspend fun recognizeFromAudio(audioData: ByteArray): Result<RecognitionResult> = runCatching {
        val base64Audio = Base64.getEncoder().encodeToString(audioData)
        
        val response = client.post("https://shazam.p.rapidapi.com/songs/v2/detect") {
            header("X-RapidAPI-Key", "")  // API key would be needed
            header("X-RapidAPI-Host", "shazam.p.rapidapi.com")
            contentType(ContentType.Text.Plain)
            setBody(base64Audio)
        }
        
        val shazamResponse = response.body<RapidApiShazamResponse>()
    ...
    /**
     * Convert RapidAPI Shazam response to our internal model
     */
    fun RapidApiShazamResponse.toRecognitionResult(): RecognitionResult? {
        ...
        val appleOption = track.hub?.options?.find {
        ...
        return RecognitionResult(
            ...
            lyrics = lyrics,
            shazamUrl = track.url,
            appleMusicUrl = appleOption?.actions?.firstOrNull()?.uri,
